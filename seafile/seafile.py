...
class SeaFileFile(io.RawIOBase):

    def __init__(self, wdfs, path, mode):
        super(SeaFileFile, self).__init__()
...
class SeaFile(FS):
    ...
    def __init__(self, url, login=None, password=None, root=None):
        self.url = url
        self.root = root
        super(SeaFile, self).__init__()

        options = {
            'seafile_hostname': self.url,
            'seafile_login': login,
...
#         return res
    ...
    def openbin(self, path, mode='r', buffering=-1, **options):
        ...
        return SeaFileFile(self, _path, _mode)
